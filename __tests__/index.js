...
describe('yorkie', () => {
  ...
  it('should support basic layout', () => {
    mkdir(dir, '.git/hooks')
    mkdir(dir, 'node_modules/yorkie-bun')
    writeFile(dir, 'package.json', '{}')

    install(dir, '/node_modules/yorkie-bun')
    const hook = readFile(dir, '.git/hooks/pre-commit')

    expect(hook).toMatch('#yorkie')
    expect(hook).toMatch('cd "."')
    expect(hook).toMatch(
      `bun run "./node_modules/yorkie-bun/src/runner.js" pre-commit`
    )
  ...
  it('should support git submodule', () => {
    ...
    uninstall(dir, 'A/B/node_modules/yorkie-bun')
    expect(exists(dir, '.git/hooks/pre-push')).toBeFalsy()
  })

  it('should not install git hooks in submodule sub directory', () => {
    mkdir(dir, '.git/modules/A/B')
    mkdir(dir, 'A/B/C/node_modules/yorkie-bun')
    writeFile(dir, 'package.json', '{}')
    writeFile(dir, 'A/B/C/package.json', '{}')
    writeFile(dir, 'A/B/.git', 'git: ../../.git/modules/A/B')

    install(dir, 'A/B/C/node_modules/yorkie-bun')
  ...
  it('should not install from /node_modules/A/node_modules', () => {
    mkdir(dir, '.git/hooks')
    mkdir(dir, 'node_modules/A/node_modules/yorkie-bun')

    install(dir, 'node_modules/A/node_modules/yorkie-bun')
    expect(exists(dir, '.git/hooks/pre-push')).toBeFalsy()
  })
  ...
  it("should not crash if there's no .git directory", () => {
    ...
    expect(() => install(dir, 'node_modules/yorkie-bun')).not.toThrow()
    expect(() => uninstall(dir, 'node_modules/yorkie-bun')).not.toThrow()
  })
